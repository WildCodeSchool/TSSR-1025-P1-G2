...
Pour une installation en une seule ligne de commande *(sinon passer à la section 2.3)* :

```bash
sudo apt install -y p7zip-full hashcat
```

**Ensuite, installer zip2hashcat :**
...
```bash
sudo wget https://github.com/hashstation/zip2hashcat/releases/download/1.0/zip2hashcat
```
...
```bash
sudo chmod u+x /usr/local/bin/zip2hashcat
```
...
**Le logiciel 7-Zip permettra de détailler le chiffrement des dossiers à attaquer.** 
...
**Détailler le chiffrement (AES, CRC, etc.) :**
...
**Télécharger l'utilitaire zip2hashcat :**

```bash
sudo wget https://github.com/hashstation/zip2hashcat/releases/download/1.0/zip2hashcat
```

**Déplacer le fichier dans le répertoire bin :**

```bash
sudo mv zip2hashcat-1.0 /usr/local/bin/
```

**S'attribuer le droit d'exécution :**

```bash
sudo chmod +x /usr/local/bin/zip2hashcat-1.0
```
---
...
#### 4.1.2 — Installation d'OpenSSH Server
...
**Lancer PowerShell en tant qu'administrateur, installer OpenSSH Server :**
...
#### 4.1.1 — Configuration du pare-feu Windows
##### Via PowerShell
...
#### 4.1.3 — Configuration du service SSH
...
#### 4.1.4 — Configuration de l'authentification par clé SSH

##### Générer une paire de clés sur Linux

```bash
ssh-keygen -t ed25519 -C "wilder@SRVWIN01"
```

**Emplacement :** /home/wilder/.ssh/id_ed25519 est par défaut

---
##### Copier la clé publique vers Windows

```bash
ssh-copy-id wilder@SRVWIN01
```

---

##### Test distant depuis Linux (Ubuntu 24.04 LTS / Debian 13.1.0)
...
Tapez **yes** et entrez le mot de passe du compte SRVWIN01.
...
#### 4.1.5 — Test de transfert de fichier (SCP)

**Récupérer un fichier depuis Linux**

```
scp wilder@SRVWIN01:C:/Users/Administrator/Dossier/fichier_1.zip /home/wilder
```
...
**Q : Vérifier que le service écoute sur le port 22**
